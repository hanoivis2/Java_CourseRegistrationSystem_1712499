...
public class EditStudentAccountForm extends JPanel implements ActionListener {
	...
	public EditStudentAccountForm(Action action, StudentAccount accountData) throws IOException, URISyntaxException {
		super(new BorderLayout());
		
		this.action = action;
		this.classId = accountData.getClassId();
		this.accountData = accountData;

		JFrame.setDefaultLookAndFeelDecorated(true);
		
        frame = new JFrame("Edit student account");
		...
		txt_Id.setText(accountData.getId());
		txt_password.setText(accountData.getPassword());
		txt_confirmPassword.setText(accountData.getPassword());
		SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
		try {
			birthdayChooser.setDate(dateFormat.parse(accountData.getBirthday()));
		} catch (ParseException e1) {
			
			e1.printStackTrace();
		}
		txt_birthplace.setText(accountData.getBirthplace());
		gender.setSelectedIndex(accountData.getGender());
		...
		if (strActionCommand.equals("Confirm"))
		{
			...
			if (txt_fullName.getText().equals("Enter student's full name...")) {
				...
				try {
					...
					int status = StudentAccountDAO.updateStudentAccount(newAccount);
					if (status == -1) {
						showMessageDialog(null, "This username has already been created!");
					}
					else {
						
						if (gender.getSelectedIndex() != accountData.getGender()) {
