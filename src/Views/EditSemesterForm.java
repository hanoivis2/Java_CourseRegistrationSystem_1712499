...
public class EditSemesterForm extends JPanel implements ActionListener {
	...
	public EditSemesterForm(Action actionFresh, Semester semesterData) throws IOException, URISyntaxException {
		...
		btn_confirm = new JButton("Edit semester");
		...
		txt_schoolYear.setEditable(false);
		...
		txt_name.setText(semesterData.getId().getName());
		txt_schoolYear.setText(semesterData.getId().getSchoolYear());
		
		SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy");
		try {
			startDateChooser.setDate(dateFormat.parse(semesterData.getStartDate()));
			endDateChooser.setDate(dateFormat.parse(semesterData.getEndDate()));
		} catch (ParseException e1) {
			
			e1.printStackTrace();
		}
		
		txt_description.setText(semesterData.getDescription());
		
		if (semesterData.getIsCurrentSemester() == 1) {
			checkBoxCurrentSemester.setSelected(true);
		}
		else {
			checkBoxCurrentSemester.setSelected(false);
		}
		...
		if (strActionCommand.equals("Confirm"))
		{
			...
			try {
				...
				if (txt_name.getText().equals("Enter subject' id...")) {
					...
					int status = SemesterDAO.updateSemester(newSemester);
					if (status == -1) {
